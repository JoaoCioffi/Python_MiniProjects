...
txtDisplay = Entry(cal,font=('arial', 12, 'bold'),
                   fg='lime',textvariable=text_Input,bd = 75,insertwidth=4,bg = 'black',
                   justify='right').grid(columnspan=4)

#-----------------------------------#
# Keys:

dx,dy = 25,25 #-> key size (x,y)

keys_general = list(range(0,10))
for i in range(0,4):   #-> keys 7,8,9 and '+' operator
    while i<3:
        key = Button(cal,padx=dx,pady=dy,bd=8,fg='black',font=('arial', 12, 'bold'),
...
for i in range(0,4):   #-> keys 0,C and '/','=' operators
    while i<3:
        ...
        if i == 1:
            key = Button(cal,padx=dx,pady=dy,bd=8,fg='white',font=('arial', 12, 'bold'),
                     text=keys[i],bg='black',command = btnClearDisplay).grid(row=4,column=i)
        elif i == 2:
            key = Button(cal,padx=dx,pady=dy,bd=8,fg='black',font=('arial', 12, 'bold'),
