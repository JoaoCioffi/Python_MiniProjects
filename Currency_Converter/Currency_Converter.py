...
root.title("GUI : Currency Conversion") #-> window title

#--- Header Object ---#
Tops = Frame(root,bg = 'lime',pady = 4, padx=4, width =4000, height = 500, relief = 'ridge') #-> header dimension
Tops.grid(row=0,column=0) #-> header position

headlabel = tk.Label(Tops,font=('lato black', 19,'bold'),
            text = '-$-   Project   :    Currency Converter  -$- ',
            bg= 'black',fg='red') #-> header content
headlabel.grid(row=1, column=0,sticky=W)


variable1,variable2 = tk.StringVar(root),tk.StringVar(root) 
...
CurrenyCode_list = ['INR','USD','CAD','CNY','DKK','EUR','BRL','GBP']
